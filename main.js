...
// Limpia el intervalo cuando el usuario cambia de página
...
document
  .getElementById("uploadHistoryBtn")
  ...
  if (encryptedMoodLog) {
    try {
      ...
      moodLog.forEach((entry) => {
        ...
        moodItem.textContent = `${new Date(
          entry.timestamp
        ).toLocaleString()}: ${entry.mood}`;
        moodLogElement.appendChild(moodItem);
      });
    } catch (error) {
      console.error(
        "Error al desencriptar el registro de estados de ánimo:",
        error
      );
    }
  }
